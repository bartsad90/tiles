import {
  ...
  displayMessage,
  renderCountScoreButton,
} from "../azul.js";
import { renderPickedTiles } from "./pickedTiles.js";
import { renderNegPointBar } from "./negPointsBar.js";
import { checkIfFinalRound } from "./mosaic.js";

export function putTilesIntoPickedRowsArray(
  pickedTiles,
  rowNumber,
  activePlayer,
  playerData,
  ...
  console.log(
    `putTilesIntoPickedRowsArray(pickedTiles, rowNumber, activePlayer, playerData), pickedTiles: `,
    pickedTiles,
    "rowNumber: ",
    rowNumber
  );
  if (!playerData[activePlayer][0].pickedRows[rowNumber][0]) {
    ...
    k = playerData[activePlayer][0].pickedRows[rowNumber]
      .map((e) => e.tileColor)
      .indexOf("blank"); //determines the index of the first blank tile in a picked row
  }
  let segregateTileOutcome;

  pickedTiles.forEach((t) => {
    let colorIsFilledInMosaicRow = false;
    playerData[activePlayer][2].mosaicArray[rowNumber].forEach((mosaicTile) => {
    ...
    if (
      playerData[activePlayer][0].pickedRows[rowNumber][rowNumber].tileColor !==
      "blank"
    ) {
      //
      let firstBlankPos = 0;
      firstBlankPos =
        playerData[activePlayer][0].pickedRows[rowNumber].findIndex(
          findFirstBlank
        );
      console.log(`First blank index in row ${rowNumber}: `, firstBlankPos);
      playerData[activePlayer][1].negPointTiles.splice(
        playerData[activePlayer][1].negPointTiles.findIndex(findFirstBlank),
        1,
        t
      );
      ...
      displayMessage(
        `Player ${activePlayer}: You already have a ${t.tileColor} tile in this mosaic row. Pick a different row.`
      );
    } else if (t.tileColor === "first-player") {
      let firstBlankPos = 0;
      ...
      displayMessage(
        `Player ${activePlayer}: ${pickedTiles.length} ${t.tileColor} tile(s) added to row ${rowNumber}`
      );
    } else if (
      !playerData[activePlayer][0].pickedRows[rowNumber]
        .map((e) => e.tileColor)
      ...
      k++;
      console.log(
        `${pickedTiles.length} ${t.tileColor} tile(s) added to row ${rowNumber}`
      );
      displayMessage(
        `Player ${activePlayer}: ${pickedTiles.length} ${t.tileColor} tile(s) added to row ${rowNumber}`
      );
  ...
  if (segregateTileOutcome === -1) {
    displayMessage(
      `Player ${activePlayer}: Too many tiles in row ${rowNumber}. Tiles moved to your penalty row.`
    );
    pickedTiles.splice(0, 5);
    isActionComplete = true;
  } else if (segregateTileOutcome === 0) {
    displayMessage(
      `Player ${activePlayer}: You picked the first-player tile. You will go first next round.`
    );
    pickedTiles.splice(0, 5);
    isActionComplete = true;
    ...
    displayMessage(
      `Player ${activePlayer}: Invalid color, pick an empty row or a row with tiles of the same color as the picked ones`
    );
...
export function renderPickedRows(
  pickedTiles,
  rowNumber,
  activePlayer,
  playerData
  ...
  console.log(
    `renderPickedRows(pickedTiles, rowNumber, activePlayer), pickedTiles: `,
    pickedTiles,
    "rowNumber: ",
    rowNumber,
    "activePlayer: ",
  ...
  document.querySelector(
    `.js-player${activePlayer}-picked-rows-container`
  ).innerHTML = innerHTML;

  //allows to choose which row the picked tiles go to

  document
    .querySelectorAll(`.js-player${activePlayer}-picked-tile-row`)
    .forEach((row) => {
      row.addEventListener("click", () => {
        while (
          !row.classList.contains(
            `js-player${activePlayer}-picked-tile-row-${rowNumber}`
          )
        ) {
          rowNumber++;
          if (rowNumber > 5) {
        ...
        let isActionComplete = putTilesIntoPickedRowsArray(
          pickedTiles,
          rowNumber,
          activePlayer,
          playerData,
...
export function generateBlankPickedRows(playerCount, playerData) {
  for (let player = 0; player < playerCount; player++) {
    let playerDataSet = playerData[player][0];
    playerDataSet.pickedRows.forEach((row) => {
      for (let j = 0; j < 5; j++) {
        row.push({ id: "defaultBlank", tileColor: "blank" });
      }
    });
    console.log(
      `blank tiles pushed into player ${player} pickedRows. playerDataSet.pickedrows: `,
      playerDataSet.pickedRows
    );
  }
}
...
export function checkPickedRowsStatus(activePlayer, playerData) {
  ...
  playerData[activePlayer][0].pickedRows.forEach((row) => {
    if (row[rowCap - 1].tileColor !== "blank") {
      pickedRowsStatus.push({
        isFilled: true,
        tileColor: row[rowCap - 1].tileColor,
      });
      console.log(`Row ${rowCap - 1} full.`);
      console.log(
        `rowCap: ${rowCap}, tileColor at rowCap-1: ${row[rowCap - 1].tileColor}`
      );
      rowCap++;
    } else {
...
export function discardFullPickedRows(discardBox, player, playerData) {
  ...
  pickedRowsStatus.forEach((row) => {
    if (row.isFilled) {
      ...
      playerDataSet.pickedRows[rowNum].forEach((tile) => {
        if (playerDataSet.pickedRows[rowNum].indexOf(tile) === 0) {
          playerDataSet.pickedRows[rowNum].splice(
            playerDataSet.pickedRows[rowNum].indexOf(tile),
            1,
            { id: "defaultBlank", tileColor: "blank" }
          ); //doesn't move one tile to discardBox to emulate the tile moved onto the mosaic
        } else {
      ...
      console.log(`pickedRow ${rowNum}: `, playerDataSet.pickedRows[rowNum]);
    }
    rowNum++;
  });
