...
export function moveWorkshopsToDiscard() {
  workshopArray.forEach((workshop) => {
    ...
    workshop.splice(0, 4);
    console.log(
      `workshop ${workshopArray.indexOf(workshop)} cleared`,
      workshop
    ); //expected for workshop to === []
  });
...
export function renderWorkshops(
  ...
  workshopArray.forEach((workshop) => {
    ...
    workshop.forEach((tile) => {
      ...
      js-workshop-${workshopArray.indexOf(workshop)}-tile-${workshop.indexOf(
        tile
      )} 
      js-tile 
      tile-${tile.tileColor}
      ">
      <img class="tile-image" src="../images/tile-${
        tile.tileColor
      }.png"></div>`;
    });
  ...
  document.querySelector(".js-workshop-container").innerHTML =
    workshopInnerHTML;
...
function addWorkshopsMouseoverEffects() {
  workshopArray.forEach((workshop) => {
    ...
    allTileColors.forEach((color) => {
      document
        .querySelectorAll(`.${color}-tile-in-workshop-${workshopNum}`)
        .forEach((tile) => {
...
export function addEventListenerWorkshop(
  pickedTiles,
  centerTable,
  activePlayer,
  playerData
  ...
  document.querySelectorAll(".js-tile").forEach((tile) => {
    tile.addEventListener("click", () => {
      ...
      const clickedWorkshop = selectTilesFromWorkshop(
        selectedColor,
        workshopNum
      );
      ...
      //clickedWorkshop[1] store tiles selected from the workshop.
      clickedWorkshop[1].forEach((tile) => {
        centerTable.push(tile);
      });

      //clickedWorkshop[0] stores tiles from the workshop which are discarded to centerTable.
      ...
      centerTable.forEach((tile) => console.log(tile));
      console.log("Picked tiles:");
      pickedTiles.forEach((tile) => console.log(tile));

      workshopArray[workshopNum].splice(
        0,
        4,
        "blank",
        "blank",
      ...
      displayMessage(
        `Player ${activePlayer}: Click an available row on your player mat to collect the tiles`
      );
