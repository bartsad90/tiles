...
export function renderMosaicGrid(playerData, activePlayer) {
  ...
  innerHTML += `</div>`;
  console.log(
    `.js-player${activePlayer}-mosaic`,
    "activePlayer: ",
    activePlayer
  );
  document.querySelector(`.js-player${activePlayer}-mosaic`).innerHTML =
    innerHTML;
}
...
export function moveTilesToMosaic(playerData, activePlayer, mosaicArray) {
  ...
  console.log("mosaicArray: ", mosaicArray);
  ...
  pickedRowsStatus.forEach((tileStatus) => {
    if (tileStatus.isFilled) {
      ...
      console.log(
        `pickedRowsStatus for row ${rowIndex}`,
        pickedRowsStatus,
        "matchingColor: ",
        matchingColor
      );
      let tileIndex = mosaicArray[rowIndex].findIndex(
        (e) => e.tileColor === matchingColor
      );
      console.log("tileIndex:", tileIndex);
      mosaicArray[rowIndex].splice(tileIndex, 1, {
        isFilled: true,
        tileColor: matchingColor,
        wasScored: false,
      });
      ...
      while (
        tileIndex - n >= 0 &&
        mosaicArray[rowIndex][tileIndex - n].isFilled === true
      ) {
        tileScore++;
        n++;
      }
      ...
      while (
        rowIndex - n >= 0 &&
        mosaicArray[rowIndex - n][tileIndex].isFilled === true
      ) {
        tileScore++;
        n++;
      }
      ...
      console.log(
        `Total points for the ${mosaicArray[rowIndex][
          tileIndex
        ].tileColor.toUpperCase()} tile in row ${rowIndex}: ${tileScore}  `
      );
...
export function checkIfFinalRound(activePlayer, playerData, isFinalRound) {
  console.log(
    "CheckIfColumnComplete(activePlayer: ",
    activePlayer,
    "playerData :",
    playerData,
    ")"
...
export function countFilledColumns(activePlayer, playerData) {
  console.log(
    `countFilledColumns(activePlayer, playerData): Counting filled columns for player ${activePlayer}`
  );
    ...
    playerData[activePlayer][2].mosaicArray.forEach((row) => {
      ...
      if (tilesInColumn === 5) {
        if (!playerData[activePlayer][5].columns.includes(column)) {
          ...
          console.log(
            "playerData[activePlayer][5].columns.push(column), column: ",
            column
          );
        }
      }
...
export function countFilledRows(activePlayer, playerData) {
  ...
  playerData[activePlayer][2].mosaicArray.forEach((row) => {
    ...
    row.forEach((tile) => {
      ...
      if (
        filledTiles === 5 &&
        !playerData[activePlayer][5].rows.includes(rowIndex)
      ) {
        playerData[activePlayer][5].rows.push(rowIndex);
        console.log(
          `Row ${rowIndex} is filled. Added rowIndex to bonus points`
        );
      }
    });
    if (filledTiles < 5) {
...
export function countCompleteColors(
  activePlayer,
  tileColorClasses,
  playerData
) {
  tileColorClasses.forEach((color) => {
    ...
    playerData[activePlayer][2].mosaicArray.forEach((row) => {
      row.forEach((tile) => {
        ...
        if (
          tilesOfSameColor === 5 &&
          !playerData[activePlayer][5].colors.includes(color)
        ) {
          playerData[activePlayer][5].colors.push(color);
          console.log(
            `Player ${activePlayer} has all 5 ${color} tiles in their mosaic`
          );
          console.log(
            `Player's bonus points array: `,
            playerData[activePlayer][5]
          );
        }
      });
    });
