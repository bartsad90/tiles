export function renderNegPointBar(playerData, activePlayer) {
  ...
  playerData[activePlayer][1].negPointTiles.forEach((tile) => {
    ...
    ">
      <div class="neg-point-indicator neg-point-indicator-${playerData[
        activePlayer
      ][1].negPointTiles.indexOf(tile)}"></div>
    <img class="tile-image" src="../images/tile-${tile.tileColor}.png"></div>
    </div>`;
  });
  document.querySelector(
    `.js-player${activePlayer}-neg-point-container`
  ).innerHTML = innerHTML;
  console.log(
    "negPointTiles: ",
...
function renderNegPointTileIndicators() {
  document.querySelectorAll(".neg-point-indicator").forEach((element) => {
    if (
      element.classList.contains("neg-point-indicator-0") ||
      element.classList.contains("neg-point-indicator-1")
    ) {
      element.innerHTML = -1;
    } else if (
      element.classList.contains("neg-point-indicator-2") ||
      element.classList.contains("neg-point-indicator-3") ||
      element.classList.contains("neg-point-indicator-4")
    ) {
      element.innerHTML = -2;
    } else if (
      element.classList.contains("neg-point-indicator-5") ||
  ...
  negPointsTiles.forEach((tile) => {
    if (tile.tileColor !== "blank") {
      if (negPointsTiles.indexOf(tile) <= 1) {
        negPoints--;
        console.log(
          `NegTileIndex: ${negPointsTiles.indexOf(
            tile
          )}, negPoints: ${negPoints}`
        );
      } else if (
        negPointsTiles.indexOf(tile) > 1 ||
        ...
        console.log(
          `Negative point tiles count: ${negTileCount}. Negative points: ${negPoints}`
        );
        console.log(
          `NegTileIndex: ${negPointsTiles.indexOf(
            tile
