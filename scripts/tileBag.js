  ...
  if (tileBag[randomTileIndex]) {
    while (randomTileIndex <= 100) {
      ...
      if (currentWorkshop > workshopCount) {
        currentWorkshop = 0;
        console.log('currentWorkshop exceeded 4. Setting currentWorkshop to: ', currentWorkshop)
      }

      if (workshopArray[currentWorkshop].length >= 4) {
        console.log(`Filled workshop ${currentWorkshop} with ${workshopArray[currentWorkshop].slice().length} tiles: `, workshopArray[currentWorkshop].slice());
        currentWorkshop++;
        console.log('currentWorkshop increased by 1:', currentWorkshop)
        console.log(`Workshop ${currentWorkshop-1} full. Switching to ${currentWorkshop}`)
    }
      ...
      if (randomTileIndex === 100) {
        ...
        if (tileBag[randomTileIndex] === 'out') {
          ...
          console.log(`object id ${randomTileIndex} found and put from tileBag to workshop ${currentWorkshop}`)
          console.log(`Workshop array after tile ${randomTileIndex} push: `, workshopArray.slice())
          break;
        }
      }
    }
...
export function checkOutCount() {
  ...
  if (outCount === workshopArray.length * 4) {
    console.log(`${workshopArray.length * 4} tiles out of the bag. Everything is spick and span`);
  } 
  else {
